...
idx = 0
found_pad_indices = set()
found_sets = 0

while (len(found_pad_indices) < 64) or (idx < (sorted(found_pad_indices)[-1] + 1000)):
    ...
    for sequence_char in find_sequence(hash_value, 5):
        ...
        for idx2 in range(max(0,idx-1000), idx):
            ...
            if sequences and sequence_char == sequences[0]:
                found_pad_indices.add(idx2)
                good_set = True
        if good_set:
            print(f"idx: {idx} found_sets: {found_sets}")
            found_sets += 1
    idx += 1
print(sorted(found_pad_indices))
print(sorted(found_pad_indices))
print(sorted(found_pad_indices)[63])
