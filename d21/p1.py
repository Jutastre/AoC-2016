...
starting_password = "abcde" if FILENAME == "tin.txt" else "abcdefgh"
    ...
    match words[0]:
        ...
        case "rotate":
            if words[1] == "left" or words[1] == "right":
                ...
                idx = password.index(words[6])
                count = 1 + idx + (1 if idx >= 4 else 0)
                password = rotate_right(password, count)
        case "reverse":
            idx1 = int(words[2])
            idx2 = int(words[4])
        ...
        case "move":
            idx1 = int(words[2])
            idx2 = int(words[5])
            temp = password.pop(idx1)
            password.insert(idx2, temp)
